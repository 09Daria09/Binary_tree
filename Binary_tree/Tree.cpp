...
Node* Tree::Edit(Node* node, const char* key)
{
	Node* temp = Search(node, key);
	cout << endl << "Enter a new correct translation of the word " << temp->m_english << endl;
	cin >> temp->GetRussian();
	node = temp;
	return node;
}
void Tree::Print_f(Node* node)
{
	if (node != nullptr)
	{
		Print_f(node->m_left);
		cout << node->m_english << '\t' << node->m_russian << endl;
		fout << node->m_english << '\t' << node->m_russian << endl;
		Print_f(node->m_right);
	}
}
void Tree::Save_file(Node* node)
{
	Node* Temp = node;
	string path;
